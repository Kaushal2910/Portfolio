...
export default function AdminPage() {
  ...
  const inputClass = "w-full p-3 rounded-lg bg-white/5 border border-white/10 text-white font-mono text-sm focus:border-white/30 outline-none";
  const btnPrimary = "px-6 py-2 rounded-lg bg-white/[0.08] text-white/70 border border-white/[0.12] hover:bg-white/[0.1] transition-all font-mono text-sm";
    ...
    <div className="min-h-screen bg-[#0a0a0a] text-white p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex gap-2 mb-8">
          <button onClick={() => { setTab('projects'); setEditing(null); setShowForm(false); }} className={`px-4 py-2 rounded-lg font-mono text-sm transition-all ${tab === 'projects' ? 'bg-white/[0.08] text-white/70 border border-white/[0.12]' : 'bg-white/5 text-gray-400 border border-white/5'}`}>
            Projects ({projects.length})
          </button>
          <button onClick={() => { setTab('certificates'); setEditing(null); setShowForm(false); }} className={`px-4 py-2 rounded-lg font-mono text-sm transition-all ${tab === 'certificates' ? 'bg-white/[0.08] text-white/70 border border-white/[0.12]' : 'bg-white/5 text-gray-400 border border-white/5'}`}>
            Certificates ({certs.length})
          </button>
        </div>

        <button onClick={() => { setShowForm(!showForm); setEditing(null); }} className="mb-6 px-4 py-2 rounded-lg bg-white/[0.06] text-white/70 border border-white/[0.12] hover:bg-white/[0.08] transition-all font-mono text-sm">
            ...
            {tab === 'projects' ? (
              <form onSubmit={handleProjectSubmit} className="space-y-4">
                ...
                <label className="flex items-center gap-2 text-sm text-gray-400 font-mono">
                  <input type="checkbox" checked={pForm.featured} onChange={e => setPForm({ ...pForm, featured: e.target.checked })} className="accent-white" /> Featured
                </label>
        ...
        <div className="space-y-3">
            ...
            <div key={p.id} className="flex items-center justify-between p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all">
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {p.featured && <span className="text-xs px-2 py-0.5 rounded bg-white/[0.08] text-white/70 font-mono">Featured</span>}
                </div>
                <p className="text-gray-500 text-sm truncate mt-1">{p.description}</p>
              </div>
              <div className="flex items-center gap-2 ml-4">
                <button onClick={() => startEdit(p)} className={btnSmall + ' hover:text-white/70'}>Edit</button>
          ...
          {tab === 'certificates' && certs.map((c) => (
            <div key={c.id} className="flex items-center justify-between p-4 rounded-xl bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all">
              ...
              <div className="flex items-center gap-2 ml-4">
                <button onClick={() => startEdit(c)} className={btnSmall + ' hover:text-white/70'}>Edit</button>
